...
function ConnectWallet(props: ConnectWalletProps) {
  ...
  const handleWalletClick = (button: ExternalButton) => {
    ...
    if (button.isInstalled) {
      ...
      // show chain namespace selector if the button has multiple chain namespaces (separate connectors per namespace).
      // MetaMask uses one multichain connector and connects all configured scopes in one session — skip the picker.
