...
export const WagmiProvider = defineComponent({
  ...
  setup(props) {
    ...
    provide(configKey, finalConfig.value);
      ...
      (newIsInitialized) => {
        if (newIsInitialized && !finalConfig.value) {
          ...
          if (finalConfig.value) {
            const { onMount } = hydrate(finalConfig.value, { ...props, reconnectOnMount: false });
            onMount();
          }
        }
      },
  ...
  render() {
    if (!this.finalConfig) return h(this.$slots.default ?? "");
    return h(Web3AuthWagmiProvider, {}, h(this.$slots.default ?? ""));
  },
});
