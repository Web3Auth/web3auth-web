...
export const getDefaultAdapterModule = async (params: {
  ...
  const { name, customChainConfig, clientId, enableLogging } = params;
  ...
  if (name === WALLET_ADAPTERS.TORUS_EVM) {
    ...
    const adapter = new TorusWalletAdapter({ chainConfig: finalChainConfig, enableLogging });
    return adapter;
  } else if (name === WALLET_ADAPTERS.TORUS_SOLANA) {
    const { SolanaWalletAdapter } = await import("@web3auth/torus-solana-adapter");
    const adapter = new SolanaWalletAdapter({ chainConfig: finalChainConfig, enableLogging });
    return adapter;
  } else if (name === WALLET_ADAPTERS.METAMASK) {
    const { MetamaskAdapter } = await import("@web3auth/metamask-adapter");
    const adapter = new MetamaskAdapter({ chainConfig: finalChainConfig, enableLogging });
    return adapter;
  } else if (name === WALLET_ADAPTERS.PHANTOM) {
    const { PhantomAdapter } = await import("@web3auth/phantom-adapter");
    const adapter = new PhantomAdapter({ chainConfig: finalChainConfig, enableLogging });
    return adapter;
    ...
    const adapter = new WalletConnectV1Adapter({ chainConfig: finalChainConfig, enableLogging });
    return adapter;
