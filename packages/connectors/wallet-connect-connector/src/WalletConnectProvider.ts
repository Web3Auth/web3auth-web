...
import { addChain, getAccounts, getProviderHandlers, switchChain } from "./walletConnectUtils";
...
export class WalletConnectProvider extends BaseProvider<BaseProviderConfig, WalletConnectProviderState, ISignClient> {
  readonly PROVIDER_CHAIN_NAMESPACE = CHAIN_NAMESPACES.EIP155;

  private connector: ISignClient | null = null;

  constructor({ config, state, connector }: { config: WalletConnectProviderConfig; state?: BaseProviderState; connector?: ISignClient }) {
    super({
