  ...
  return (
    <div className="w3ajs-passwordless w3a-group w3a-group--passwordless">
      <div className="w3a-group__title">
        {t("modal.social.passwordless-title")}
      ...
      <form className="w3ajs-passwordless-form" onSubmit={(e) => handleFormSubmit(e)}>
        <input
          ...
          required
          placeholder={`${t("modal.social.sms-placeholder-text")}: +(00)123456/name@example.com`}
          onFocus={(e) => (e.target.placeholder = "")}
          onBlur={(e) => (e.target.placeholder = `${t("modal.social.sms-placeholder-text")}: +(00)123456/name@example.com`)}
          onChange={(e) => handleInputChange(e)}
        />

        {isValidInput === false && <div className="w3a-sms-field--error">{t("modal.errors-invalid-number-email")}</div>}

        <button disabled={fieldValue === ""} className="w-full w3a-button" type="submit">
