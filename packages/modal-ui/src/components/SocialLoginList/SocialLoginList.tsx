...
export type rowType = {
  method: SocialLoginsConfig['loginMethods'],
  isDark: boolean,
  isPrimaryBtn: boolean,
  name: string,
  adapter: SocialLoginsConfig['adapter'],
  loginParams: { loginProvider: SocialLoginsConfig['loginMethods'], name: string, login_hint: string },
  order: number,
...
const SocialLoginList = (props: SocialLoginListProps) => {
  ...
  createEffect(() => {
    ...
    Object.keys(props.socialLoginsConfig.loginMethods).filter((method) => {
    ...
    <div class="w3a--flex w3a--flex-col w3a--w-full w3a--gap-y-2">
      <Show when={mainOption()}>
        <SocialLoginButton
          ...
          isDark={isDark}
          isPrimaryBtn={mainOption().isPrimaryBtn} onClick={() =>
          ...
          <For each={visibleRow()}>
            {(row) =>
              <SocialLoginButton
                ...
                method={row.method}
                isDark={isDark}
                isPrimaryBtn={row.isPrimaryBtn} onClick={() =>
                  props.handleSocialLoginClick({
                    adapter: row.adapter,
      ...
      <Show when={canShowMore()}>
        <button type="button" class="w3a--text-xs w3a--font-normal w3a--text-app-primary-600 hover:w3a--text-app-primary-500 dark:w3a--text-app-primary-500 dark:hover:w3a--text-app-primary-600 w3a--text-right" onClick={handleExpand}>{expand() ? 'View Less' : 'View More'}</button>
      </Show>
