<template>
	<div class="container">
		<div class="sidebar">
			<a href="https://github.com/Web3Auth/Web3Auth/tree/master/demo/vue-app-new" target="blank">
			...
			<div class="flex-vertical-center plugins">
				...
				<span class="form-control radio-group">
					<label for="walletServices" class="radio-button">
						<input id="walletServices" v-model="form.plugins.walletServices" type="checkbox"
							value="wallet services plugin" />
			...
			<div v-if="authMode === 'hosted'" class="hosted">
				<div class="ui-mode">
					<div class="flex-vertical-center ui-mode">
						...
						<span class="form-control radio-group">
							<label for="sapphire_mainnet" class="radio-button">
								<input type="radio" id="sapphire_mainnet" value="sapphire_mainnet" v-model="openloginNetwork" />
								Sapphire Mainnet
							</label>
							<label for="sapphire_devnet" class="radio-button">
								<input type="radio" id="sapphire_devnet" value="sapphire_devnet" v-model="openloginNetwork" />
								Sapphire Devnet
							</label>
							<label for="mainnet" class="radio-button">
								<input id="mainnet" v-model="openloginNetwork" type="radio" value="mainnet" />
								Mainnet
							</label>
							<label for="testnet" class="radio-button">
								<input id="testnet" v-model="openloginNetwork" type="radio" value="testnet" />
								Testnet
							</label>
							<label for="cyan" class="radio-button">
								<input id="cyan" v-model="openloginNetwork" type="radio" value="cyan" />
								Cyan
							</label>
						</span>
					</div>
				...
				<div class="flex-vertical-center chain">
					...
					<span class="form-control radio-group">
						<label for="ethereum" class="radio-button">
							<input id="ethereum" v-model="chain" type="radio" value="ethereum" />
							Ethereum
						</label>
						<label for="solana" class="radio-button">
							<input id="solana" v-model="chain" type="radio" value="solana" />
							Solana
						</label>
						<label for="binance" class="radio-button">
							<input id="binance" v-model="chain" type="radio" value="binance" />
							Binance
						</label>
						<label for="polygon" class="radio-button">
							<input id="polygon" v-model="chain" type="radio" value="polygon" />
							Polygon
						</label>
					</span>
				</div>
					...
					<div class="flex-vertical-center ui-mode">
						...
						<span class="form-control radio-group">
							<label for="default" class="radio-button">
								<input id="default" v-model="selectedUiMode" type="radio" value="default" />
								Default
							</label>
                ...
                CustomUI
							...
							<label for="whitelabel" class="radio-button">
								<input id="whitelabel" v-model="selectedUiMode" type="radio" value="whitelabel" />
								WhiteLabel
							</label>
						</span>
					</div>
					...
					<!-- <div v-if="form.selectedUiMode === 'customUi'">
						<div class="flex-vertical-center">
							...
							<span class="form-control">
								<input id="openlogin" v-model="form.uiMode.customUi.type" type="radio" name="openlogin"
									value="openlogin" />
					...
					<!-- <div v-if="form.selectedUiMode === 'whitelabel'">
						...
						<div class="order-container">
							<div class="form-label">
								...
								<a @click="setDefaultLoginMethodsOrder" @keydown="setDefaultLoginMethodsOrder">Set to
									default</a>
							</div>
							<div>
			...
			<div v-else-if="authMode === 'ownAuth'" class="ownAuth">
		...
		<div class="content">
			<section>
				<!-- hosted auth -->
				<!-- <ConfigurableExample v-if="config.selectedUiMode === 'default' && config.authMode === 'hosted'"
					:plugins="form.plugins" :openlogin-network="config.openloginNetwork" :adapter-config="config.uiMode.default"
					:chain="config.chain" />

				<WhitelabelExample v-else-if="config.selectedUiMode === 'whitelabel' && config.authMode === 'hosted'"
					:ui-config="config.uiMode.whitelabel" :chain="config.chain" /> -->

				<!-- Custom auth -->
				<!-- <CustomUiContainer v-if="config.authMode === 'ownAuth'" :auth-type="config.uiMode.customUi.type">
				</CustomUiContainer> -->
			</section>
		</div>
	</div>
...
import { ref } from "vue";
import { RadioGroup } from "@toruslabs/vue-components";

const authModes = [
	{ label: "Hosted", id: "hosted", value: "hosted" },
	{ label: "Own Auth", id: "ownAuth", value: "ownAuth" },
];

const authMode = ref<string>("hosted");
const openloginNetwork = ref<string>("sapphire_mainnet");
const chain = ref<string>("ethereum");
const selectedUiMode = ref<string>("default");
