...
import { authConnector, Web3AuthNoModal } from "@web3auth/no-modal";
...
function App() {
  ...
  const onLogin = async (response: CredentialResponse) => {
    ...
    const provider = await web3auth?.connectTo<AuthLoginParams>(WALLET_CONNECTORS.AUTH, { loginProvider: "google", login_hint: payload.email, extraLoginOptions: { id_token: idToken }, mfaLevel: "none" });
    if (provider) {
